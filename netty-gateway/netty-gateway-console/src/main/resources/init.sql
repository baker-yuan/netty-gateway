...
(
    `serviceId`  varchar(255) NOT NULL COMMENT '服务唯一id',
    `basePath`   varchar(255) DEFAULT '' COMMENT '服务URL前缀，全局唯一',
    `protocol`   varchar(255) DEFAULT '' COMMENT '服务的具体协议 http、dubbo、grpc',
    `enable`     tinyint(1)   DEFAULT '1' COMMENT '服务启用禁用',
    `invokerMap` text COMMENT '服务方法信息 key=invokerPath(完整路径) value=服务调用的接口模型描述',
...
(
    `serviceInstanceId` varchar(255) NOT NULL COMMENT '服务实例ID(ip:port)',
    `serviceId`         varchar(255) DEFAULT '' COMMENT '服务定义唯一id',
    `address`           varchar(255) DEFAULT '' COMMENT '服务实例地址(ip:port)',
    `tags`              text COMMENT '标签信息',
    `weight`            int(11)      DEFAULT '100' COMMENT '权重信息',
    `registerTime`      bigint(20)   DEFAULT '0' COMMENT '服务注册的时间戳，后面我们做负载均衡，warmup预热',
