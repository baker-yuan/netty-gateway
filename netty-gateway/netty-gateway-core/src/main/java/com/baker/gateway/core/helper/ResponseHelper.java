...
public class ResponseHelper {
	...
	public static FullHttpResponse getHttpResponse(ResponseCode responseCode) {
		GatewayResponse resGatewayResponse = GatewayResponse.buildGatewayResponse(responseCode);
		DefaultFullHttpResponse httpResponse = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, 
				HttpResponseStatus.INTERNAL_SERVER_ERROR,
				Unpooled.wrappedBuffer(resGatewayResponse.getContent().getBytes()));
	...
	/**
	 * 通过上下文对象和GatewayResponse对象 构建FullHttpResponse
	 */
	private static FullHttpResponse getHttpResponse(Context ctx, GatewayResponse gatewayResponse) {
		ByteBuf content;
